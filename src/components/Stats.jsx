// Секция «Активность»: карточки Codewars и GitHub с живыми данными из API.
// Данные загружаются в useEffect и вставляются напрямую в DOM через innerHTML —
// так проще строить динамичные таблицы без дополнительного React-стейта.
...
// Возвращает цвет для бейджа ранга Codewars по принятой цветовой схеме сайта:
// 7–8 kyu = серый, 5–6 kyu = жёлтый, 3–4 kyu = синий, 1–2 kyu = фиолетовый, dan = белый
...
export default function Stats({ t }) {
  // Запрашивает данные из /api/codewars и /api/github-activity, затем рендерит карточки через innerHTML.
  // Скелетоны (animate-pulse) показываются до прихода данных — они уже вёрстаны в JSX ниже.
  useEffect(() => {
    // Запрос к Vercel serverless function, которая проксирует Codewars API
    fetch('/api/codewars')
      .then(r => r.json())
      .then(cw => {
    ...
    // Запрос к Vercel serverless function, которая возвращает { profile, repos, events } из GitHub API
